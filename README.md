...
* Post window is displayed in a regular vim buffer
  ...
  * Toggles back if hidden on a Tidal error
* Interactive argument hints in the command-line area
* Status line widgets
  * Display Tidal server status in vim status line.
* Snippet generator
...
#### 2. Install the SCNvim Tidal classes

The Tidal classes are located in the `sc/` directory of this repo.

This directory needs to be linked to your Tidal `Extensions` directory (run `Platform.userExtensionDir` in Tidal to see where it is located on your platform).
...
* **macOS**
    ...
    ln -s ~/.config/nvim/plugged/scnvim/sc ~/Library/Application\ Support/Tidal/Extensions/scide_scvim/scnvim
    ```
...
* **Linux**
    ...
    ```shell
    # create scide_scvim directory
    mkdir -p $HOME/.local/share/Tidal/Extensions/scide_scvim
    
    # create a symbolic link to 'sc' in the 'scide_scvim' directory named 'scnvim'
    ln -s /home/<USER>/.config/nvim/plugged/scnvim/sc /home/<USER>/.local/share/Tidal/Extensions/scide_scvim/scnvim
    ```

    If the above symlink does not work (you can tell this by opening up a
    Tidal file and running `:SCNvimStart` - if something's wrong the
...
Open a new file in `nvim` with a `.scd` or `.sc` extension and type `:SCNvimStart` to start Tidal.

## Syntax highlighting

Run `:SCNvimTags` after starting Tidal to generate a file used for syntax highlighting.
...
Tidal, where `<port>` is the UDP port of the remote plugin. Currently
there is no way to support multiple (Tidal) sessions without guessing
the port number for the remote plugin. The `:SCNvimStatusLine` command ensures
that Tidal always connects to the correct port.
...
| Command | Description |
| --- | --- |
| `SCNvimStart` | Start Tidal |
| `SCNvimStop` | Stop Tidal |
...
" set this variable to browse Tidal documentation in nvim (requires `pandoc`)
...
```plain
scnvim - Tidal integration for Neovim
Copyright © 2018-2019 John Murphy
